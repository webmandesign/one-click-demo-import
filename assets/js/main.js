	...
	function ajaxCall( data ) {
		...
		.done( function( response ) {
			...
			if ( 'undefined' !== typeof response.status && 'newAJAX' === response.status ) {
				...
				$( '.js-ocdi-ajax-response' ).append( '<div class="notice notice-error"><p>' + response + '</p></div>' );
				$( '.js-ocdi-ajax-loader' ).hide();
			}
		})
		.fail( function( error ) {
			$( '.js-ocdi-ajax-response' ).append( '<div class="notice notice-error"><p>Error: ' + error.statusText + ' (' + error.status + ')' + '</p></div>' );
			$( '.js-ocdi-ajax-loader' ).hide();
		});
	}
